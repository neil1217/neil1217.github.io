...
var _INLINE_TAG_MAP = _toMap('a,abbr,acronym,b,basefont,bdo,big,br,button,cite,code,del,dfn,em,font,i,img,input,ins,kbd,label,map,q,s,samp,select,small,span,strike,strong,sub,sup,textarea,tt,u,var'),
	...
	_NOSPLIT_TAG_MAP = _toMap('html,head,body,td,tr,table,ol,ul,li,a,img'),
...
KEditor.prototype = {
	...
	text : function(val) {
		var self = this;
		if (val === undefined) {
			return _trim(self.html().replace(/<(?!embed|video|source).*?>/ig, '').replace(/&nbsp;/ig, ' '));
		} else {
	...
	count : function(mode) {
		...
		if (mode === 'text') {
			return self.text().replace(/<(?:embed|video|source).*?>/ig, 'K').replace(/\r\n|\n|\r/g, '').length;
		}
		return 0;
	},
	exec : function(key) {
...
KindEditor.plugin('clearhtml', function(K) {
    ...
    self.clickToolbar(name, function() {
		...
		// 步骤 1: 提取所有的表格并存储在 tableContents
		let tableRegex = /<table[^>]*>[\s\S]*?<\/table>/g;
		let tableCounter = 1;
		let tableContents = {};  // 存储表格内容

		// 提取表格内容并替换为占位符
		html = html.replace(tableRegex, (match) => {
		let tableKey = `tableee${tableCounter}`;
		tableContents[tableKey] = match;  // 存储表格内容
		return `此处插入表格${tableCounter++}`;  // 替换为占位符
		});
		...
		html = html.replace(/<h1>\s*活动详情\s*<\/h1>/g, '<strong>活动详情<\/strong>');
		html = html.replace(/<h2>\s*活动详情\s*<\/h2>/g, '<strong>活动详情<\/strong>');
		html = html.replace(/<h3>\s*活动详情\s*<\/h3>/g, '<strong>活动详情<\/strong>');
		html = html.replace(/<h4>\s*活动详情\s*<\/h4>/g, '<strong>活动详情<\/strong>');
		html = html.replace(/<h5>\s*活动详情\s*<\/h5>/g, '<strong>活动详情<\/strong>');
		html = html.replace(/<h6>\s*活动详情\s*<\/h6>/g, '<strong>活动详情<\/strong>');
		html = html.replace(/<p>\s*活动详情：\s*<\/p>/g, '<strong>活动详情：<\/strong>');
	        html = html.replace(/<h1>\s*活动详情：\s*<\/h1>/g, '<strong>活动详情：<\/strong>');
	  	html = html.replace(/<h2>\s*活动详情：\s*<\/h2>/g, '<strong>活动详情：<\/strong>');
	  	html = html.replace(/<h3>\s*活动详情：\s*<\/h3>/g, '<strong>活动详情：<\/strong>');
		...
		html = html.replace(/<p>\s*活动规则\s*<\/p>/g, '<strong>活动规则<\/strong>');
	    	html = html.replace(/<h1>\s*活动规则\s*<\/h1>/g, '<strong>活动规则<\/strong>');
	  	html = html.replace(/<h2>\s*活动规则\s*<\/h2>/g, '<strong>活动规则<\/strong>');
	  	html = html.replace(/<h3>\s*活动规则\s*<\/h3>/g, '<strong>活动规则<\/strong>');
	 	html = html.replace(/<h4>\s*活动规则\s*<\/h4>/g, '<strong>活动规则<\/strong>');
	 	html = html.replace(/<h5>\s*活动规则\s*<\/h5>/g, '<strong>活动规则<\/strong>');
	   	html = html.replace(/<h6>\s*活动规则\s*<\/h6>/g, '<strong>活动规则<\/strong>');
		html = html.replace(/<p>\s*活动规则：\s*<\/p>/g, '<strong>活动规则：<\/strong>');
	   	html = html.replace(/<h1>\s*活动规则：\s*<\/h1>/g, '<strong>活动规则：<\/strong>');
	  	html = html.replace(/<h2>\s*活动规则：\s*<\/h2>/g, '<strong>活动规则：<\/strong>');
	  	html = html.replace(/<h3>\s*活动规则：\s*<\/h3>/g, '<strong>活动规则：<\/strong>');
		...
		html = html.replace(/<p>\s*活动细则\s*<\/p>/g, '<strong>活动细则<\/strong>');
	    	html = html.replace(/<h1>\s*活动细则\s*<\/h1>/g, '<strong>活动细则<\/strong>');
	  	html = html.replace(/<h2>\s*活动细则\s*<\/h2>/g, '<strong>活动细则<\/strong>');
	  	html = html.replace(/<h3>\s*活动细则\s*<\/h3>/g, '<strong>活动细则<\/strong>');
	 	html = html.replace(/<h4>\s*活动细则\s*<\/h4>/g, '<strong>活动细则<\/strong>');
	 	html = html.replace(/<h5>\s*活动细则\s*<\/h5>/g, '<strong>活动细则<\/strong>');
	   	html = html.replace(/<h6>\s*活动细则\s*<\/h6>/g, '<strong>活动细则<\/strong>');
		html = html.replace(/<p>\s*活动细则：\s*<\/p>/g, '<strong>活动细则：<\/strong>');
	   	html = html.replace(/<h1>\s*活动细则：\s*<\/h1>/g, '<strong>活动细则：<\/strong>');
	  	html = html.replace(/<h2>\s*活动细则：\s*<\/h2>/g, '<strong>活动细则：<\/strong>');
	  	html = html.replace(/<h3>\s*活动细则：\s*<\/h3>/g, '<strong>活动细则：<\/strong>');
		...
		html = html.replace(/<p>\s*规则与条款\s*<\/p>/g, '<strong>规则与条款<\/strong>');
	    	html = html.replace(/<h1>\s*规则与条款\s*<\/h1>/g, '<strong>规则与条款<\/strong>');
	  	html = html.replace(/<h2>\s*规则与条款\s*<\/h2>/g, '<strong>规则与条款<\/strong>');
	  	html = html.replace(/<h3>\s*规则与条款\s*<\/h3>/g, '<strong>规则与条款<\/strong>');
		...
		function adjustTableWidth(tableContents) {
			...
			tableContents = tableContents.replace(/\n+/g, '\n');
			tableContents = tableContents.replace('<table>', '<table width="100%" cellspacing="0" cellpadding="0" border="0" class="otable">');
			//tableContents = tableContents.replace(/<p>\s*<\/p>/g, ''); // 移除空的 <p> 标签
		...
		if (maxTdRow) {
			...
			widthPerCell = Math.floor(widthPerCell * 10) / 10;  // 保证每个单元格的宽度精确到小数点后一位
		...
		html = html.replace(/此处插入表格(\d+)/g, (match, number) => {
			let tableKey = `tableee${number}`;
			let tableHtml = tableContents[tableKey];
			
			// 格式化并调整表格宽度
			let adjustedTableHtml = adjustTableWidth(tableHtml);
			
			return adjustedTableHtml || match;  // 将调整后的表格 HTML 插回
		  });
...
*清理文字代码
*******************************************************************************/
KindEditor.plugin('textclear', function(K) {
    var self = this, name = 'textclear';    
	self.clickToolbar(name, function() {
        self.focus();
        var html = self.html();
...
KindEditor.plugin('imgurl11', function(K) {
	...
	self.clickToolbar(name, function() {
		...
		self.html(html); 		
		 ...
		 copyButton.addEventListener('click', function() {	
			resultContainer.innerText = self.html();
			resultContainer.innerText = resultContainer.innerText.replace(/&lt;/g, '<');
			resultContainer.innerText = resultContainer.innerText.replace(/&gt;/g, '>');
			html = html.replace(/&gt;/g, '>');				
			 var range = document.createRange();
